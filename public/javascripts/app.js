import { Subject, fromEvent, merge, combineLatest, BehaviorSubject } from 'rxjs';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const dataUpdate$ = new BehaviorSubject(null);
    ...
    function loadRecipes$() {
        return ajax.getJSON('/api/recipes?_=' + Date.now());
    }
    ...
    // Главный поток отображения рецептов
    combineLatest([
        dataUpdate$,
        search$,
        sort$,
    ...
    fromEvent(recipeForm, 'submit').pipe(
        ...
        switchMap(recipeData => {
            ...
            return ajax({
                ...
                tap(() => dataUpdate$.next(Date.now()))
            );
        })
    ).subscribe();

    // Делегирование событий на кнопки edit/delete
